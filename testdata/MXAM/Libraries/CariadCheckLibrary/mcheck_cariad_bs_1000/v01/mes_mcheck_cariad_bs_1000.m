%% mcheck_cariad_bs_1000 - Cariad brand
%
% OUTPUT:
% =======
...
% PASS/FAIL CRITERIA:
% ===================
% 1. The library block must be masked.
% 2. The icon units must be set to normalized
% 3. The mask drawing commands must contain the branding function
%
% SOLUTION:
% =========
...
%
%% IMPLEMENTATION
function [result,cr_items,r_stats] = mes_mcheck_cariad_bs_1000(system,cmd_s)
...
try
    ...
    for ix = 1 : numel(cs_blocks)      
        ...
        % check icon units, must be 'normalized'
        if ~strcmp(objMask.IconUnits, 'normalized')
            % build check item structure for current rule violation:
            r_item = mxam_newcritem(defaultItem,...
                'message', 'The property IconUnits must be ''normalized''.', ...
                'handle', hBlock, ...
                'path', mes_cleanpathname(hBlock), ...
                'name', mes_cleanName(hBlock));
            cr_items{end+1} = r_item; % add new check item to result list
        ...
        % check mask icon drawing command
        if ~contains(objMask.Display, 'evalBrandingCommand')
            % build check item structure for current rule violation:
            r_item = mxam_newcritem(defaultItem,...
                'message', 'The mask draw commands do not call ''evalBrandingCommand''.', ...
                'handle', hBlock,...
                'path', mes_cleanpathname(hBlock),...
                'name', mes_cleanName(hBlock));
            cr_items{end+1} = r_item; % add new check item to result list            
