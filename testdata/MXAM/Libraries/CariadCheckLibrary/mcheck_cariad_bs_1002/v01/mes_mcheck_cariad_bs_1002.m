%% mcheck_cariad_bs_1002 - Ports must be labeled
%
% OUTPUT:
% =======
...
% PASS/FAIL CRITERIA:
% ===================
% Fail: The MaskHideContents property must have the value 'on'
%
% SOLUTION:
% =========
...
%
%% IMPLEMENTATION
function [result,cr_items,r_stats] = mes_mcheck_cariad_bs_1002(system,cmd_s)
...
try
    ...
    for ix = 1 : numel(cs_blocks)      
        ...
        % check MaskHideContents property, must be 'on'
        actualValue = get_param(refBlock, 'MaskHideContents');
        if ~strcmp(actualValue, 'on')
            % build check item structure for current rule violation:
            r_item = mxam_newcritem(defaultItem,...
                'message', 'The property MaskHideContents must be ''on''.', ...
                'handle', hBlock, ...
                'path', mes_cleanpathname(hBlock), ...
                'name', mes_cleanName(hBlock));
            cr_items{end+1} = r_item; % add new check item to result list
